...
`preflight` is read-only. Without `--input`, it emits the role's exact skeleton, enums, constraints, and canonical evaluator factor names where applicable. With `--input` and the intended `--job-id`, it invokes the same canonical validator used for completion and returns its first RFC 6901 pointer diagnostic plus the raw input hash, canonical artifact hash, and contextual receipt while leaving the state, event log, and attempt counters unchanged. There is no separately maintained preflight validator. The main agent supplies `--original-input` for every input preflight. This compares parsed authored content and returns the original byte hash; retain that result outside managed state with the original response. Research permits only claim/source reference repairs within commercial-evidence categories and the control assessment, with all claims, source records, statuses and other content fixed. The canonical validator still checks repaired references, and the same author must justify their relevance. Candidate redesigns also permit repair of the two derived changed-field declaration lists: canonical validation recomputes the exact normalized fingerprint and commercial-container differences against the immutable parent. All underlying candidate fields, evidence, economics and economic-effect rationale remain fixed; the same constructor justifies the metadata repair. All kinds also permit same-author trimming of surrounding string whitespace, with internal whitespace, words, numbers, classifications, and structure unchanged. Other kinds otherwise permit only JSON formatting differences. A content change or unreadable original blocks a receipt and is not a schema retry. This comparison is an orchestration prerequisite; historical receipt formats and existing publications are unchanged. Correct only mechanical schema defects and preserve semantic content. After a successful preflight, start the job and complete it with the same bytes, reported input digest, and reported receipt digest. Structured completion requires both digests; its event stores the full receipt, and `check` rejects missing or inconsistent evidence. `validate` remains the concise authoritative validator for callers that do not need the role contract; it does not replace the preflight-before-start workflow.

For missing evidence in an invalid, unadmitted research response, the explicit `--research-supplement` preflight mode requires both `--input` and `--original-input`. It permits appended source records with distinct new URLs, appended claims that each cite a new source, and appended commercial/control references. Original source and claim lists must remain exact prefixes; every other original value, including status, adverse findings and falsification mappings, stays identical. The result must still pass canonical validation and the configured source limit. A valid original, already admitted research, changed original content or removed reference is rejected. The response reports added source/claim IDs alongside the two input hashes; preserve this audit outside managed state. This is a bounded evidence-completion attempt under the existing mechanical budget, not permission to reconsider a valid judgment. A fresh candidate-only researcher receives the original failed response and diagnostic with its normal founder/candidate bindings; source relevance and actual research must be verified from its tool evidence. Neither the original-comparison result nor attempt history before job start is bound into the historical completion receipt format; preserving and auditing them remains the main agent's responsibility.
...
This output includes only that batch's candidate refs and artifacts plus immutable founder and evaluator bindings. Each binding is a path relative to the run directory and a SHA-256, not the snapshot contents. Pass the run directory explicitly in the assignment and resolve every returned artifact or snapshot path against it; for example, `inputs/founder.md` means `<run-directory>/inputs/founder.md`. The assigned evaluator must read exactly those two files, verify both hashes, and use their contents; an isolation prompt must expressly permit those reads while prohibiting other repository context. It omits other batches, scores, rankings, portfolio state, unrelated jobs, campaign state, and history.
